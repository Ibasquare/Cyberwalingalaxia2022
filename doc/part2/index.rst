...
.. warning:: 
   if you are using the setup described in section :ref:`Connect to a remote VM (Kali) already setup <target connect to a remote VM>`, you need to specify the flag ``--config=~/inetsim.conf`` when running ``inetsim`` to avoid any conflict with other users.
